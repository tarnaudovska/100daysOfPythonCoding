logo = """ __        __   _                            _ 
 \ \      / /__| | ___ ___  _ __ ___   ___  | |
  \ \ /\ / / _ \ |/ __/ _ \| '_ ` _ \ / _ \ | |
   \ V  V /  __/ | (_| (_) | | | | | |  __/ |_|
    \_/\_/ \___|_|\___\___/|_| |_| |_|\___| (_)☕ machine"""
...
def process_coins(coffee):
    ...
    print(f"Insert coins.The {coffee} is {cost}")
...
def is_transaction_successful(money_recieved, drink):
    ...
    if money_recieved >= MENU[drink]["cost"]:
        ...
        print(f'⚠️ Sorry that\'s not enough money.💵 Money refund. {money_recieved} are not enough. The coffee costs {MENU[drink]["cost"]}$')
        return False
...
is_machine_on = True
while is_machine_on:
    promt = input('What would you like? ☕ Espresso,latte or cappuccino?\n').lower() 
    if promt == "espresso" or promt == "cappuccino" or promt == "latte":
        print("Please pay for the coffee")
        
        if has_resources(promt) == True and is_transaction_successful(process_coins(promt), promt) == True:
